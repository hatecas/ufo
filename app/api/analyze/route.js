...
【用語定義 — 必ずこの用語を使い分けよ】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
・받침봉(バー): 景品が乗っている横棒(2本の平行な棒)。ユーザーの位置案内の基準になる。
  → 韓語出力時: 「받침봉」「앞쪽 받침봉」「뒤쪽 받침봉」と書くこと。
  → 「봉」だけで済ませるな。必ず「받침봉」と明記。
・집게(アーム/クロー): UFOキャッチャーの上から降りてくる掴み装置。
  → 韓語出力時: 「집게」と書くこと。
・집게발(アーム先端): 集計の先端部分。ゴム足が付いていることもある。
  → 韓語出力時: 「집게발」と書くこと。

★ visual_guide を書くとき — 初心者向け「現場型ガイド」ルール:
  ✗ "뒤쪽 받침봉에서 바깥쪽으로 7cm" ← 현장에서 cm를 잴 수 없다！
  ✗ "target_x=45%" ← 사용자에게 무의미한 숫자
  ✓ "박스 뒷면 끝선에 맞춰서" ← 사진에서 바로 확인 가능
  ✓ "뒤쪽 받침봉 바로 위, 박스가 봉에서 벗어나기 시작하는 지점" ← 지형지물 기반
  ✓ "박스 로고 왼쪽 끝 바로 위" ← 사진에 보이는 랜드마크 활용

★ action を書くとき — 動作のニュアンスまで伝えよ:
  ✗ "밀기" ← 너무 추상적
  ✓ "집게가 내려가면서 박스 뒷면을 스치듯 눌러서 앞으로 밀어내기" ← 구체적 동작
  ✓ "집게발 한쪽이 박스 모서리를 살짝 걸어서 들어올리기" ← 어떻게 힘이 전달되는지

★ tip を書くとき — 조준 멈춤 타이밍을 시각적으로:
  ✗ "집게를 너무 깊이 넣지 마세요" ← 얼마나가 깊은지 모름
  ✓ "집게발이 박스 뒷면 선과 일치할 때 버튼을 누르세요" ← 멈추는 순간이 명확
  ✓ "집게 중심이 뒤쪽 받침봉 바로 위에 왔을 때 내리세요" ← 기준점이 명확
...
C. オーバーハング(はみ出し)の実測比較:
   ...
   - d_front = 앞쪽 받침봉(前方バー)から景品前端までの距離(cm推定)
   - d_back = 뒤쪽 받침봉(後方バー)から景品後端までの距離(cm推定)
   - d_left = 左側 받침봉から景品左端までの距離
   - d_right = 右側 받침봉から景品右端までの距離
...
例:
  "A1(22%,28%)〜A2(68%,72%)。앞쪽 받침봉 A3≈y58%。
   d_max方向=前方(앞쪽 받침봉에서 앞으로 6cm > 뒤쪽 받침봉에서 뒤로 3cm)。
   攻略点=景品前端の中央やや右。
   target_x = 22 + (68-22)×0.6 = 49.6% → 50%
   target_y = A2のy - 3% = 69%
   visual_guide: 좌우: 박스 앞면 로고의 중앙보다 약간 오른쪽, 앞뒤: 박스 앞면 끝선 바로 위 (앞쪽 받침봉보다 더 앞)"
...

{
  ...
  "physics_engine": {
    "anchors": {
      "bars": "받침봉 인식: 위치(앞쪽/뒤쪽), 간격, 고무 유무",
      "overhang_comparison": "앞쪽 받침봉→앞 d_front=Xcm, 뒤쪽 받침봉→뒤 d_back=Ycm → d_max 방향=[앞/뒤/좌/우]"
    },
  ...
  "next_move": {
    "action": "구체적 동작 묘사 (예: '집게가 내려가면서 박스 뒷면을 스치듯 눌러 앞으로 밀어내기')",
    "visual_guide": "좌우: [사진에 보이는 랜드마크 기반 — 박스 로고/모서리/받침봉 교차점 등], 앞뒤: [받침봉과 박스가 만나는 선/박스 끝선 등 지형지물 기반]",
    "force_type": "push / lift / torque",
    "expected_movement": "예상 움직임 묘사 (예: '박스가 앞쪽으로 살짝 밀리면서 앞쪽 받침봉 위에 더 걸치게 됨')",
    "target_x_percent": 50,
    "target_y_percent": 50
  },
  "situation_analysis": {
  ...
  "tip": "조준 멈춤 타이밍을 시각적으로 설명 (예: '집게발이 박스 뒷면 선과 일치할 때 버튼을 누르세요')"
}
