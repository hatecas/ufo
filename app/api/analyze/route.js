...

{
  ...
  "steps": [
    {
      "step": 1,
      "action": "한줄 행동 요약 (10글자 이내, 예: '왼쪽 모서리 밀기')",
...
★ 各ステップは3つの別々のフィールドに分けて記述すること（重要！detailは使わない）：
  - "where": 集計を降ろす正確な位置（1文で）
  - "mechanism": 集計が閉じる時に何が起きるか（1文で）
  - "expected_result": この動きで景品がどう変わるか（1文で）

★ 各フィールドの記述ルール：
  - "where" には「アームの中心をどこに合わせるか」を書く。景品のどの部分の真上か。
  - "mechanism" には「どちらの爪が景品に当たるか」「閉じる力でどう押されるか」を書く。
  - "expected_result" には「予想される変化」を具体的に書く（〇度回転、〇cm移動など）。

★ 記述例：
BAD (1つのdetailに全部書く): "박스의 왼쪽 모서리를 오른쪽으로 밀기. 무게중심이..."
GOOD (3つに分ける):
  "where": "박스 왼쪽 끝에서 약 1cm 왼쪽에 집게 중심을 맞추세요"
  "mechanism": "집게가 닫힐 때 오른쪽 발만 박스 왼쪽 모서리에 걸려서 오른쪽으로 밀어냅니다"
  "expected_result": "박스가 시계방향으로 약 10~15도 회전합니다"
